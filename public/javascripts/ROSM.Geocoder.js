ROSM.Geocoder = {
  ...
  showGeoResultOnHidden: function(marker_id, response) {
    ...
    if(response.results.length == 0) {
      document.getElementById("geocoder-result").innerHTML = "検索結果は見つかりませんでした。検索を絞り込んでください。";
      return;
    }
  ...
  showGeoResultOnSearch: function(response) {
    var s = ""
    if(response.results.length == 0) {
      document.getElementById("search-point-result").innerHTML = "検索結果は見つかりませんでした。検索を絞り込んでください。";
      return;
    }
    for(var i = 0; i < response.results.length; i++) {
      var x = [];
